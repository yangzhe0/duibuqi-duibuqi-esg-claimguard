...
class FormalV2Tests(unittest.TestCase):
    def test_current_formal_v2_pool_is_submission_ready(self):
        base = Path(__file__).resolve().parents[1] / "outputs/formal_v2"
        pool_path = base / "indicator_pool_v2.csv"
        self.assertTrue(pool_path.is_file())
        self.assertTrue((base / "indicator_pool_v2.json").is_file())
        self.assertTrue((base / "indicator_selection_report.md").is_file())
    ...
    def test_formal_v2_sample_runner_writes_required_schema_with_fake_llm(self):
        from scripts.run_esg_formal_v2 import run_sample
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            report_dir = root / "data/parsed_reports_v1/reports/000001_测试公司_2025_ESG报告"
            ...
            pool_dir = root / "outputs/formal_v2"
    ...
    def test_formal_v2_parser_handles_fenced_json_and_repairs_quantitative_unit(self):
        from scripts.run_esg_formal_v2 import _normalize_v2_result
...
结束"""
    ...
    def test_formal_v2_quantitative_found_without_number_is_marked_incomplete(self):
        from scripts.run_esg_formal_v2 import _normalize_v2_result
        from src.esg_demo.indicators import Indicator
        ...
        self.assertEqual(row["quantitative_incomplete"], "true")
    ...
    def test_zero_event_unknown_unit_does_not_fabricate_unit(self):
        ...
        self.assertEqual(row["postprocess_repaired"], "false")
        self.assertEqual(row["quantitative_incomplete"], "true")
        self.assertEqual(row["repair_reason"], "zero_event_but_unit_unknown")
    ...
    def test_formal_v2_sample_runner_retries_invalid_json_once(self):
        from scripts.run_esg_formal_v2 import run_sample
    ...
    def test_formal_v2_cli_script_is_directly_executable_from_project_root(self):
        result = subprocess.run(
            [sys.executable, "scripts/run_esg_formal_v2.py", "--help"],
        ...
        self.assertIn("Run formal_v2 qwen3 ESG extraction", result.stdout)

    def test_formal_v2_50_script_exists(self):
        self.assertTrue((Path(__file__).resolve().parents[1] / "scripts/run_esg_formal_v2_50.py").is_file())

    def test_formal_v2_100_script_exists(self):
        self.assertTrue((Path(__file__).resolve().parents[1] / "scripts/run_esg_formal_v2_100.py").is_file())

    def test_formal_v2_200_and_system_entrypoints_exist(self):
        root = Path(__file__).resolve().parents[1]
        self.assertTrue((root / "scripts/run_esg_formal_v2_200.py").is_file())
    ...
    def test_formal_v2_runner_accepts_explicit_report_paths_and_resume(self):
        from scripts.run_esg_formal_v2 import run_sample
    ...
    def test_formal_v2_audit_normalizes_labels_and_writes_outputs(self):
        from scripts.review_formal_v2_50_audit import run_audit
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._write_csv(base / "extraction_results.csv", [row])
            self._write_csv(base / "sample_review.csv", [row])
            (base / "extraction_results.json").write_text(json.dumps([row], ensure_ascii=False), encoding="utf-8")

            summary = run_audit(base_dir=base, max_rows=1, client=FakeJudge())

            self.assertEqual(summary["reviewed_count"], 1)
            self.assertTrue((base / "sample_review_ai_audited.csv").is_file())
            self.assertTrue((base / "audit_metrics_by_indicator.csv").is_file())
            self.assertTrue((base / "pre_100_fix_suggestions.csv").is_file())
            with (base / "sample_review_ai_audited.csv").open(encoding="utf-8-sig") as fh:
                audited = list(csv.DictReader(fh))
            self.assertEqual(audited[0]["audit_label"], "correct")
            self.assertEqual(audited[0]["audit_error_type"], "none")
