...
class FormalPipelineTests(unittest.TestCase):
    def test_empty_vlm_table_is_backfilled_from_same_page_model_bbox(self):
        import scripts.run_esg_formal as formal_pipeline

        with tempfile.TemporaryDirectory(dir=formal_pipeline.PROJECT_ROOT / "outputs") as tmp:
            root = Path(tmp)
            report_id = "r-table"
            parsed = formal_pipeline.canonical_json(root, report_id)
            ...
            summary = formal_pipeline.repair_empty_vlm_tables(root, {"reports": [{"report_id": report_id}]})
    ...
    def test_failed_qwen_preflight_is_archived_and_retried(self):
        ...
        with tempfile.TemporaryDirectory(dir=formal_pipeline.PROJECT_ROOT / "outputs") as tmp:
            ...
            with (
                mock.patch.object(formal_pipeline, "_load_indicators", return_value=[indicator]),
                mock.patch.object(formal_pipeline, "load_content_list", return_value=[[{}]]),
                mock.patch.object(formal_pipeline, "flatten_report", return_value=[{}]),
                mock.patch.object(formal_pipeline, "select_candidate_blocks", return_value=candidates),
                mock.patch.object(formal_pipeline, "_build_evidence_prompt", side_effect=["x" * size for size in range(100, 106)]),
                mock.patch.object(formal_pipeline, "build_llm_client", return_value=FakeClient()),
                mock.patch.object(formal_pipeline, "llama_token_count", side_effect=range(100, 106)),
            ):
                payload = formal_pipeline.run_qwen_preflight(
                    root,
                    manifest,
                    "http://unused",
    ...
    def test_safe_output_root_rejects_protected_trees(self):
        from scripts.run_esg_formal import PROJECT_ROOT, safe_output_root

        with self.assertRaises(ValueError):
            safe_output_root(PROJECT_ROOT / "data/new")
        with self.assertRaises(ValueError):
            safe_output_root(PROJECT_ROOT / "data/new")
        accepted = safe_output_root(PROJECT_ROOT / "outputs/formal_pipeline_test")
        self.assertEqual(accepted, (PROJECT_ROOT / "outputs/formal_pipeline_test").resolve())

    def test_valid_content_list(self):
        from scripts.run_esg_formal import valid_content_list
    ...
    def test_extraction_resume_contract_detects_parsed_change(self):
        from scripts.run_esg_formal import PROJECT_ROOT, canonical_json, ensure_extraction_contract
        ...
        with tempfile.TemporaryDirectory(dir=PROJECT_ROOT / "outputs") as tmp:
            ...
            with mock.patch("scripts.run_esg_formal.runtime_fingerprints", return_value={"model": {"sha256": "c" * 64}}):
    ...
    def test_validation_accepts_exact_contract(self):
        from scripts.run_esg_formal import EXPECTED_REPORTS, INDICATOR_POOL, _read_csv, sha256, validate_all
        ...
        with tempfile.TemporaryDirectory(dir=Path(__file__).resolve().parents[1] / "outputs") as tmp:
            ...
            with mock.patch(
                "scripts.run_esg_formal.validate_data001_corrections",
                ...
                validation = validate_all(
                    root,
                    {
                        ...
                        "reports": reports,
                        "indicator_pool": {
                            "path": "outputs/final_results/indicator_pool.csv",
