...
| 要求 | 状态 | 当前证据 | 下一动作 |
|---|---|---|---|
| 可运行的 AI 原型 | 已具备 | 生产 smoke test 18 个端点通过 | 保持一键验收进入提交包 |
| AI inference 效果与运行指标 | 已具备 | V3：200 份报告、10015 次 Qwen3.6 生成、错误 0；9806 条保留候选行具有可用行级耗时 | 在项目文档中与准确率指标分栏呈现 |
| 准确率声明边界 | 已具备 | 本作品仅声明工程完整性和证据可追溯性，不使用工程复核记录计算准确率 | 不声明未经独立人工评测的 Precision、Recall 或 F1 |
| 创新性论证 | 已具备 | 项目文档和创新矩阵已集中说明四项机制、现有证据、待验证增益与失败退化方案 | 保持工程证据与效果指标边界 |
...
| 5 分钟以内项目视频 | 已具备 | outputs/ai_contest/submission/final/队不起队不起_ESG ClaimGuard_项目视频.mp4、outputs/ai_contest/submission/supporting/队不起队不起_ESG ClaimGuard_无声视频.mp4 | 无字幕画面、独立配音和真实工作台录屏均须通过最终抽帧复核 |
...
- Qwen3 实际调用：10015 次
- LLM / 结果错误：0 / 0
- 有候选且保留计时的最终结果行：9806 条
- 这些行的平均 / P95 耗时：2.777 / 3.718 秒
