...
def main() -> int:
    ...
    def build(cache_dir: Path) -> list[dict[str, object]]:
        cache_dir.mkdir(parents=True, exist_ok=True)
        segments, generated_timings = asyncio.run(synthesize(cache_dir, args.voice))
        build_master(segments, args.output)
        return generated_timings

    if args.cache_dir is None:
        with tempfile.TemporaryDirectory(prefix="claimguard-audio-") as temporary:
            timings = build(Path(temporary))
    else:
        timings = build(args.cache_dir)

    (args.output.parent / "项目视频口播时序.json").write_text(
        json.dumps(timings, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
    (args.output.parent / "项目视频口播.txt").write_text(
