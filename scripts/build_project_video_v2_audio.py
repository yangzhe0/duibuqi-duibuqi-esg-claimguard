...
SCENES = [
    ...
    },
    {
        ...
        "rate": "-3%",
        "pitch": "-1Hz",
        "text": "现在看到的是项目的真实工作台。进入系统总览后，可以先查看报告规模、指标分布和处理状态。环境、社会与治理三个维度的覆盖情况，以及每份报告的 found 数量，都可以快速比较。再进入披露预审，按照报告、指标和风险维度筛选候选；候选队列、抽取值和处置状态会同步联动。点击一条记录，就能继续查看结构化结果和证据位置。整个过程不是让模型直接下结论，而是把值得复核的内容，连同出处一起送到使用者面前。质量摘要和新报告接入，也在同一个界面中完成。",
    },
    {
        "start": 137.6,
        "rate": "-3%",
    ...
    },
    {
        ...
        "rate": "-2%",
        "pitch": "-2Hz",
        "text": "完整推理在单卡顺序部署下完成，总墙钟约七点七六小时。一万零十五次生成调用没有模型调用错误；冻结文件的校验和全部通过，六十三项 Python 测试和十六项生产 smoke 也全部通过。换句话说，我们交付的不只是一次演示，而是一条可以重新运行、重新核对的工程链路。",
    },
    {
        "start": 237.8,
        "rate": "-1%",
