...
DEFAULT_POOL = Path("outputs/formal_v2/indicator_pool_v2.csv")
DEFAULT_OUT_DIR = Path("outputs/formal_v2/llm_sample")
...
def run_sample(
    ...
    llm = client or OllamaClient(model=model, url=ollama_url)
    results = _load_existing_results(out_dir / "extraction_results.csv") if resume else []
    ...
    summary = {
        ...
        "previous_results_loaded": len(completed),
...
def _normalize_v2_result(report_id: str, indicator: Indicator, raw_text: str, candidate_count: int, elapsed_seconds: float) -> dict:
    try:
...
def _diagnostics_title(path: Path) -> str:
    if path.name == "llm_50_diagnostics.md":
        return "# formal_v2 LLM 50 Diagnostics"
    if path.name == "llm_100_diagnostics.md":
        return "# formal_v2 LLM 100 Diagnostics"
    if path.name == "llm_200_diagnostics.md":
        return "# formal_v2 LLM 200 Diagnostics"
    return "# formal_v2 LLM Sample Diagnostics"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run formal_v2 qwen3 ESG extraction for parsed MinerU reports.")
