...
from src.esg_demo.formal_extraction import _build_evidence_prompt, _load_indicators, _normalize_result, run_sample
...
def safe_output_root(path: Path) -> Path:
    ...
    if PROJECT_ROOT.resolve() not in resolved.parents:
        raise ValueError(f"formal output root must stay inside the project: {resolved}")
    return resolved
    ...
    manifest.update(
        {
            ...
            "extraction_config": {
                "prompt_version": "evidence-only-20260822",
                "canonical_postprocess": "model-table-bbox-backfill",
                "max_candidates": 5,
                "context_tokens": 8192,
...
def verify_protected_inputs(manifest: dict) -> None:
    if sha256(INDICATOR_POOL) != manifest["indicator_pool"]["sha256"]:
        raise RuntimeError("Indicator pool changed after the formal cohort was frozen")
    for row in manifest["reports"]:
...
def build_manifest(root: Path) -> dict:
    ...
    if manifest_path.is_file():
        ...
        if not isinstance(manifest, dict) or len(manifest.get("reports", [])) != EXPECTED_REPORTS:
            raise RuntimeError("Existing formal manifest is malformed or not a frozen 200-report manifest")
        return manifest
    ...
    manifest = {
        "schema_version": "formal-final-run-manifest-1",
        "created_at": utc_now(),
...
def repair_empty_vlm_tables(root: Path, manifest: dict) -> dict:
    ...
    summary = {
        "schema_version": "formal-final-table-backfill-1",
        "updated_at": utc_now(),
...
def extraction_contract_payload(root: Path, manifest: dict) -> dict:
    ...
    return {
        "schema_version": "formal-final-extraction-contract-1",
...
def validate_all(root: Path, manifest: dict) -> dict:
    ...
    if not validation["passed"]:
        raise RuntimeError("formal validation failed: " + json.dumps(checks, ensure_ascii=False))
    return validation


def comparison_boundary(root: Path) -> dict:
    """Record the comparison boundary without carrying process-era artifacts."""
    return {"comparison_only_not_accuracy": True, "independent_benchmark_used": False}
...
def main() -> int:
    ...
    try:
        ...
        raise RuntimeError(f"Another formal pipeline process already holds {root / '.run.lock'}") from exc
    lock_stream.seek(0)
    ...
    try:
        ...
        comparison = comparison_boundary(root)
