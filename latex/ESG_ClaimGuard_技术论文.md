...
冻结队列由 `input_manifest.csv` 选取 200 份公开 ESG 类报告，共 10,528 页；项目目录中的其他 PDF 不属于该正式队列。指标池含环境 25、社会 20、治理 20 项，形成 13,000 个 report×indicator 任务。正式运行使用 MinerU2.5-Pro-2605-1.2B 和 Qwen3.6-27B Q4_K_M。本项目将 Qwen 官方仓库中的 Qwen3.6-27B 以 Q4_K_M 量化形态部署，仅使用文本输入和 8,192 token 运行上下文；模型身份与许可依据官方模型卡记录 [2]。209 条被重试熔断器保留的记录由 Codex Agent 模拟人工工程复核，其中 found 193、missing 16；该集合不作为独立测试集。
